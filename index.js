...
function get20s(array){
  ...
  for (let i=0; i < array.length; i++){
    let livingYears = array[i].years.split(" - ");
    if (livingYears[0].includes("19") && livingYears[1].includes("19")){
    twentiethCArray.push(array[i].name);
  }
}
return twentiethCArray;
}
...
function addArtist(array){
    array.push({
      ...
      genre: "Web Design", 
      nationality: "USA",
      bio: "You may say I'm a dreamer, but I'm not the only one. I hope some day you'll join us, and the world will be as one. -JLennon"
    });
    return array;
  }
